...
**纯 Kotlin 原生 Android AI 智能体。** 一个运行在手机上的自主 AI Agent:多供应商接入、工具调用循环、子智能体并行协作、在线插件市场、技能场景自动引用、长期记忆、内置 root/Ubuntu 环境与终端,以及像素风「智能体指挥室」实时动画。从零构建,无跨端框架。

> 版本:**1.14.1**(versionCode 136) · 许可:**GPL-3.0-or-later**
...
- **运行链** — 长任务通知栏一键中断/审批批复(无回执不执行);多轮任务自动继承前文结论;运行流水落盘,杀进程可续跑、可分支重试;工具结果必回模型,失败明示不静默。
- **计划 / 协作模式** — 计划模式可视化任务卡;协作模式下主脑把任务并行派发给多个专职子智能体,汇总回主脑;内置四个专职角色,误删后升级自动找回。
...
- **插件与技能** — 在线插件市场(27 个实测可用:气象/金融/知识/影音/工具/饮食/游戏/开发 8 分类,云端上架免发版,SSRF 出站守卫,Key 只进请求头);技能场景自动引用(命中附用法)+ `/技能名` 主动调用;MCP 支持(stdio 本地传输 + HTTP),`@服务器名` 优先使用其工具。
- **安全** — 权限模式(询问 / 完全访问)、敏感操作确认卡(通知栏可批)、审计日志;执行策略判定与权限求交已打底。
